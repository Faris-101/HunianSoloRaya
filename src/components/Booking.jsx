...
export default function Booking({ onSubmit }) {
  ...
  const inputCls = "w-full px-4 py-3 bg-white/5 border border-white/10 font-sans text-[0.88rem] text-white outline-none transition-colors duration-200 focus:border-gold resize-none placeholder:text-white/25";
  const labelCls = "block text-[0.68rem] tracking-[0.14em] uppercase text-white/35 mb-2";

  return (
    <section id="booking" className="px-5 py-14 bg-dark text-white md:px-8 md:py-20 lg:px-16 lg:py-24">
      ...
      <div className="mb-8 lg:hidden">
        ...
        <div className="py-5 border-y border-white/8">
          <p className="text-[0.65rem] tracking-[0.14em] uppercase text-white/30 mb-4">Chat Langsung via WhatsApp</p>
          <WAButtons message="Halo, saya ingin info lebih lanjut tentang hunian di Hunian SoloRaya" size="md" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_1.8fr] lg:gap-20">
        ...
        <div className="hidden lg:flex flex-col justify-center">
          ...
          <div className="pt-8 border-t border-white/8">
            <p className="text-[0.65rem] tracking-[0.14em] uppercase text-white/30 mb-5">Chat Langsung via WhatsApp</p>
            <WAButtons message="Halo, saya ingin info lebih lanjut tentang hunian di Hunian SoloRaya" size="md" />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input className={inputCls} type="text" placeholder="Nama Anda" value={form.nama} onChange={e => set("nama", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>Nomor HP / WA</label>
              <input className={inputCls} type="tel" placeholder="+62 8xx xxxx xxxx" value={form.hp} onChange={e => set("hp", e.target.value)} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select className={inputCls + " appearance-none cursor-pointer"} value={form.proyek} onChange={e => set("proyek", e.target.value)}>
                <option value="" className="bg-dark">Pilih proyek...</option>
                {PROJECTS.map(p => (
                  <option key={p.slug} value={p.slug} className="bg-dark">{p.name}</option>
                ))}
              </select>
            </div>
            <div>
          ...
          <button
            onClick={() => {
              ...
              window.open(`https://wa.me/6287888701191?text=${encodeURIComponent(pesan)}`, "_blank");
              onSubmit();
              setForm(INIT);
            }}
