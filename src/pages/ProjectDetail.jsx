  ...
  return (
    <div className="bg-cream min-h-screen">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 py-4 bg-cream/95 backdrop-blur-xl border-b border-[rgba(26,24,20,0.1)] md:px-8 md:py-5 lg:px-16">
        ...
        <a
          href="/"
          className="font-serif text-[1.3rem] font-medium tracking-[0.04em] text-dark no-underline md:text-[1.5rem]"
        >
          Hunian<span className="text-gold">SoloRaya</span>
        </a>
        <a
          href="/#booking"
          className="px-4 py-2 bg-dark text-cream text-[0.68rem] tracking-[0.08em] uppercase no-underline hover:bg-gold transition-colors duration-200 md:px-5 md:py-3 md:text-[0.75rem]"
        >
          Booking
        </a>
      </nav>
      ...
      {lightbox && (
        ...
        >
          ...
          <button
            onClick={() => setLightbox(false)}
            className="absolute top-4 right-4 text-white/60 hover:text-white text-[1.5rem] bg-transparent border-0 cursor-pointer leading-none"
          >
            ✕
          </button>
          {project.photos.length > 1 && (
            <>
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 text-white/60 hover:text-white text-[2.5rem] bg-transparent border-0 cursor-pointer leading-none"
              >
                ‹
              </button>
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-white/60 hover:text-white text-[2.5rem] bg-transparent border-0 cursor-pointer leading-none"
              >
                ›
              </button>
            </>
          )}
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/40 text-[0.75rem] tracking-[0.1em]">
      ...
      <div className="px-5 py-8 max-w-[900px] mx-auto md:px-8 md:py-12 lg:px-16 lg:py-16 flex flex-col gap-10 md:gap-14">
        ...
        <div>
          ...
          <h1 className="font-serif text-[clamp(1.8rem,5vw,3.5rem)] font-light leading-[1.1] mb-2">
            {project.name}
          </h1>
          ...
          <div className="flex items-center justify-between gap-4 py-4 border-y border-[rgba(26,24,20,0.08)] md:hidden">
            <div>
              <div className="text-[0.65rem] tracking-[0.1em] uppercase text-gray mb-0.5">
                {project.statusLabel}
              </div>
              <div className="font-serif text-[1.3rem] text-gold">
                {project.price}
              </div>
            </div>
            <WAButtons
              message={`Halo, saya tertarik dengan ${project.name}. Boleh info lebih lanjut?`}
              size="sm"
            />
          </div>
        </div>
        ...
        {/* Deskripsi */}
        <div>
          <h3 className="text-[0.7rem] tracking-[0.18em] uppercase text-gray mb-4 pb-3 border-b border-[rgba(26,24,20,0.08)]">
            Tentang Proyek
          </h3>
          <p className="text-[0.9rem] text-gray leading-[1.8]">
            {project.description}
          </p>
        </div>
          ...
          <div className="grid grid-cols-3 gap-px bg-[rgba(26,24,20,0.08)]">
            {["jumlah_unit", "tipe_unit", "sertifikat"].map((key) => (
              <div key={key} className="bg-cream py-4 px-3 md:py-5 md:px-4">
                <div className="text-[0.6rem] tracking-[0.1em] uppercase text-gray mb-1.5 md:text-[0.65rem]">
                  {specLabels[key]}
                </div>
                <div className="font-serif text-[0.95rem] font-light text-dark md:text-[1.1rem]">
                  {project.specs[key]}
                </div>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-px sm:grid-cols-2">
            {project.units_detail.map((u, i) => (
              <div
                key={i}
                className="relative overflow-hidden p-6 text-white border border-gold/10 hover:border-gold/35 transition-colors duration-300 group md:p-8"
                style={{
              ...
              >
                ...
                <div className="font-serif text-[1.2rem] font-light text-[#f5f0e8] mb-2">
                  {u.name}
                </div>
                ...
                <div className="font-serif text-[1.1rem] text-gold-light pt-3 border-t border-gold/15">
                  <small className="font-sans text-[0.62rem] text-white/28 block mb-1 tracking-[0.08em] uppercase">
                    Mulai dari
                  </small>
                  {u.price}
                </div>
                <a
                  href="/#booking"
                  className="inline-block mt-4 text-[0.65rem] tracking-[0.12em] uppercase text-gold no-underline hover:opacity-70 transition-opacity duration-200"
                >
                  Tanyakan Unit Ini →
                </a>
              </div>
        ...
        <div className="hidden md:block py-6 border-y border-[rgba(26,24,20,0.08)]">
          <p className="text-[0.65rem] tracking-[0.14em] uppercase text-gray mb-5">
          ...
          <div className="flex flex-col gap-5 sm:grid sm:grid-cols-2 sm:gap-8">
            ...
            >
              <div className="w-full h-full flex flex-col items-center justify-center gap-4 bg-gradient-to-br from-[#2a2520] to-[#1a1814]">
                <div
                  className="w-10 h-10 bg-gold"
                  style={{
                    clipPath: "polygon(50% 100%,0 40%,20% 0,80% 0,100% 40%)",
                  }}
                />
                <p className="text-white/40 text-[0.78rem] tracking-[0.08em] text-center px-5">
                  {project.address}
                </p>
              </div>
              <button
            ...
            <div>
              <div className="text-[0.7rem] tracking-[0.15em] uppercase text-gray mb-3">
                Akses Terdekat
              </div>
            ...
            .map((p, i) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 project-overlay text-white md:p-6">
                  <div className="text-[0.6rem] tracking-[0.12em] uppercase text-gold mb-1">
                    ● {p.statusLabel}
                  </div>
                  <div className="font-serif text-[1rem] mb-0.5 md:text-[1.2rem]">
                    {p.name}
                  </div>
                  <div className="text-[0.72rem] text-white/55 hidden sm:block">
                    {p.location}
                  </div>
                  <div className="font-serif text-[0.85rem] text-gold/80 mt-1">
                    {p.price}
                  </div>
                </div>
              </div>
